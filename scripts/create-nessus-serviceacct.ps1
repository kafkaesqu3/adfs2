...
# create account
Try {
New-ADUser -SamAccountName $identity -GivenName "nessus scanner service account" -Surname "NESSUS SVC" -Name $identity `
...
$servicePrincipalName = 'NESSUS/' + $hostname + '.' + $Domain.dnsroot + '@' + $Domain.dnsroot.ToUpper()
